...
def compare_bn(bn1, bn2):
    ...
    if bn1.track_running_stats and bn2.track_running_stats:
        if not torch.allclose(torch.flatten(bn1.running_mean), torch.flatten(bn2.running_mean)):
            print('Diff in running_mean: {} vs {}'.format(
                torch.flatten(bn1.running_mean), torch.flatten(bn2.running_mean)))
            err = True

        if not torch.allclose(torch.flatten(bn1.running_var), torch.flatten(bn2.running_var)):
            print('Diff in running_var: {} vs {}'.format(
                torch.flatten(bn1.running_var), torch.flatten(bn2.running_var)))
            err = True
...
def main():
    ...
    for track_running_stats in [True, False]:
        for manual_update in [True, False]:
            ...
            # train mbn
            mbn = MaskedBatchNorm1d(num_features, affine=True, track_running_stats=track_running_stats, 
                                    manual_update=manual_update)
            # train bn
            bn = nn.BatchNorm1d(num_features, affine=True, track_running_stats=track_running_stats)
            for _ in range(runs):
                ...
                if manual_update and track_running_stats:
                    mbn.update_masked_running_stats(input_mask=torch.ones(B, 1, L))
                compare_bn(mbn, bn)
            # eval
            mbn.eval()
            bn.eval()
            for _ in range(runs):
                x = torch.randn(B, num_features, L)
                out_mbn = mbn(x)
                out_bn = bn(x)
                # assert(torch.allclose(out_mbn.data, out_bn.data))
                print(torch.allclose(out_mbn.data, out_bn.data), (out_mbn - out_bn).abs().max().data)
