...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, _) in enumerate(dataloader):
        ...
        while fake_imgs.size()[0] < real_imgs.size()[0]:
            fake_imgs_ = generator(z)
            mr_mask = compute_memorization_rejection_mask(fake_imgs_, t=mrt).view(-1, 1, 1, 1)
            fake_imgs_ = torch.masked_select(fake_imgs_, mr_mask).view(-1, 3, 32, 32)
